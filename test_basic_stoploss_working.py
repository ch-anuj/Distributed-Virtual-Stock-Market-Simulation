...
def createUserOrders(rumour, userList, orderType, marketPrice):        
...
def getMatchOrders(userList, sellersBuyers, orderType, userOrders, marketPrice, userStopOrdersSell, userStopOrdersBuy):
    ...
    ## different returns are listed so as to check the correction of the code
...
def newLocalMarketP(listM,currP):
    ...
    lenMatch = len(listM)
    if lenMatch != 0:
        ## these are the different ways with which one can determine the new prices.
        ## I've used the random choice method because it generates more real looking
        ## graph than the other methods.
...
def avgGlobalMarketP(globalP):
    ...
    globalP.sort()
    if lengmp != 0:
        ## these are the different ways with which one can determine the new global
        ## prices, I've used the median method for determining the global prices
        ## because it generates more real looking
        ## graph than the other methods.
        
        # ngmp = globalP[-1]
        ngmp = stats.median(globalP)

        # ngmp = stats.mode(globalP)

        # ngmp = rd.choice(globalP)

        # k = rd.randint(1,10)
        # list1 = globalP[ 1 :: int(lengmp/3) ]
        # list2 = globalP[ (int(lengmp/3)+1) :: ( int(lengmp*(2/3) )+1 )]
        # list3 = globalP[ ( int(lengmp*(2/3) )+1 ) :: lengmp]
        #
        # if k == 1 or k == 2:
        #     if list2 is None:
        #         ngmp = rd.choice(globalP)
...
def setNewCurrP(currP,newGlobalP):
    ...
    return currP
## for broadcasting local prices and after calling this function each node will have the local prices of all other node
## an algorithm will decide new global prices as written in the function avgGlobalMarketP()
...
def startMarket(highLow,currP):
    ...
    userOrder = createUserOrders("none", userList, orderType, currP)
    ...
    # can see the difference between local prices and the global prices using print(newLocalP, currP) => broadcasting working fine
    globalP.clear()
    #print("##############################")
